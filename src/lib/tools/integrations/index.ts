 ...
 * - **Companies House** uses the byte-for-byte ported LTM client at
 *   `src/lib/companies-house/client.ts` (production-grade rate-limit
 *   retry, full surface coverage). Q3 wrapper is thin.
 * - **Stripe** is OB-original Q3 metrics; LTM's `lib/stripe.ts` (billing
 *   subscription sync) is a different concern, future port post-Track F.
 * - **GitHub / LinkedIn / QuickBooks / Xero** confirmed absent in LTM —
 *   OB-original.
 * - **Supabase** confirmed: LTM uses Supabase as DB client only, no Q3
 *   metrics surface — OB-original.
 */
