...
| W-009 ✅ | LTM documents subsystem entanglement — port surfaced 3 missed LTM utility files (`@/types/blocks`, `@/lib/autolinker`, `@/lib/documents/content`); Clerk dependencies in BookmarkButton + DocumentActionBar; OrgMapProvider used by 4 blocks (not 2 as manifest claimed); react-markdown + remark-gfm dependencies; DocumentRenderer routes break when any block defers; cannot ship clean without Clerk strategy | 2026-05-03 Session 7 attempted port | **RESOLVED** across 4 sub-sessions: **Session 8** atoms (commit `8d30887`) — 18 blocks + DocumentRenderer + DocumentBody + 3 utility files + OrgMapProvider + DocumentWorkspace data spine + lib/studio/engine namespaced. **Session 8b** workspace-shell-atoms (commit `2abec1a`) — DocumentWorkspace component + 4 sibling components. **Session 8b-routes** data layer (commit `241cc89`) + LTM-alignment fix (commit `4a1ef53`) — 4 LTM-aligned Prisma models + `prisma/sql/08-add-documents-engine-write-surface.sql` + 6 API routes. **Session 8b-routes-components** (commit `3356279`) — 14 LTM components (BookmarkButton, DocumentActionBar, SaveToPackageModal + AddToPackageButton + PackageProgressBar, ShareDocumentModal, SaveToMyDocumentsButton + useSaveToMyDocuments hook, DocumentSourcePanel, DocumentFilters, DocumentCard, DocumentQuickView, DocumentQuickViewProvider, PrintButton) — verbatim ports with one OB-only adaptation (`useRef<T>()` → `useRef<T \| undefined>(undefined)` for React 19 / TS 6 strictness in DocumentActionBar). Documents UX rendering chain complete from atoms through workspace shell through write-surface routes through write-surface components. **927/927 across 85 suites.** Inline regex-divergence comment in OB's `computeBlockStatus` + `computeFieldStatus` documents an OB-only bug fix from S8 (LTM source untouched). **Operator action owed:** apply `prisma/sql/08-add-documents-engine-write-surface.sql` to Supabase. **What still defers (Session 8d):** Document Prisma model + UserProfile model + 13 documents app routes + the real fork logic to replace the 501 stub at `/api/me/documents/save-from-template`. |
