...
| W-009 ▲ partial | LTM documents subsystem entanglement — port surfaced 3 missed LTM utility files (`@/types/blocks`, `@/lib/autolinker`, `@/lib/documents/content`); Clerk dependencies in BookmarkButton + DocumentActionBar; OrgMapProvider used by 4 blocks (not 2 as manifest claimed); react-markdown + remark-gfm dependencies; DocumentRenderer routes break when any block defers; cannot ship clean without Clerk strategy | 2026-05-03 Session 7 attempted port | **PARTIAL Session 8 (commit `8d30887`):** atoms-only port shipped — 18 blocks + DocumentRenderer + DocumentBody + 3 missed utility files + OrgMapProvider (default-empty `orgMap` makes the autolinker resolve external URLs only) + DocumentWorkspace.tsx data-spine extract (types + helpers, lines 1–208 of LTM source) + lib/studio/engine namespaced (types, entityModes, questionMapper). `react-markdown` + `remark-gfm` installed. 24 new tests across 2 suites; 899/899 passing. Inline regex-divergence comment added to OB's `computeBlockStatus` and `computeFieldStatus` — fixed an LTM `lastIndex`-stateful bug in copies (LTM source untouched). **Workspace shell** (DocumentEditor, DocumentFieldEditor, BookmarkButton, DocumentActionBar, SaveToPackageModal, ShareDocumentModal, etc., 16 files) **and app routes** (`app/documents/*`, 13 files per § K.4 step 6) **remain deferred** to a follow-up session — they need OB-side bookmark/package/share API routes that don't exist yet. |
