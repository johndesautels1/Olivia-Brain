...
| W-009 ✅ | LTM documents subsystem entanglement — port surfaced 3 missed LTM utility files (`@/types/blocks`, `@/lib/autolinker`, `@/lib/documents/content`); Clerk dependencies in BookmarkButton + DocumentActionBar; OrgMapProvider used by 4 blocks (not 2 as manifest claimed); react-markdown + remark-gfm dependencies; DocumentRenderer routes break when any block defers; cannot ship clean without Clerk strategy | 2026-05-03 Session 7 attempted port | **RESOLVED** across 5 sub-sessions: **Session 8** atoms (commit `8d30887`) — 18 blocks + DocumentRenderer + DocumentBody + 3 utility files + OrgMapProvider + DocumentWorkspace data spine + lib/studio/engine namespaced. **Session 8b** workspace-shell-atoms (commit `2abec1a`) — DocumentWorkspace component + 4 sibling components. **Session 8b-routes** data layer (commit `241cc89`) + LTM-alignment fix (commit `4a1ef53`) — 4 LTM-aligned Prisma models + `prisma/sql/08-add-documents-engine-write-surface.sql` + 6 API routes. **Session 8b-routes-components** (commit `3356279`) — 14 LTM components (BookmarkButton, DocumentActionBar, SaveToPackageModal + AddToPackageButton + PackageProgressBar, ShareDocumentModal, SaveToMyDocumentsButton + useSaveToMyDocuments hook, DocumentSourcePanel, DocumentFilters, DocumentCard, DocumentQuickView, DocumentQuickViewProvider, PrintButton). **Session 8d data foundation** (commits `2cb414e` schema + `502db7f` fork logic) — 12 LTM-aligned enums + Document + UserProfile Prisma models + 4 FK constraints on the 08 tables + Package outreachGoal/packageStatus enum tightening + `ensureUserProfile()` helper at `src/lib/users/` + real fork logic at `/api/me/documents/save-from-template` (replaces the S8b-routes 501 stub). Documents UX rendering chain + write-surface routes + Document/UserProfile data layer complete. **929/929 across 85 suites.** Inline regex-divergence comment in OB's `computeBlockStatus` + `computeFieldStatus` documents an OB-only bug fix from S8 (LTM source untouched). One OB-only React 19 useRef adaptation in DocumentActionBar. **Operator actions owed (5 SQL files in apply order):** 06 → seed → 07 → 08 → 09. **What still defers (Session 8d-routes):** the 13 `app/documents/*` route pages (`documents/page.tsx`, `documents/saved`, `documents/new`, `documents/[id]`, `documents/[id]/edit`, `documents/[id]/workspace/*`, `documents/[id]/studio/*`, loading/error). |
...
| W-015 ✅ | Clerk auth STUB at `src/lib/auth/session.ts`. C4 routes (Twilio user-facing endpoints) call `getAuthSession()` instead of Clerk's `auth()`. Stub reads `STUB_USER_ID` env var in dev/preview, throws clearly when env unset OR in production. NOT a band-aid — throws-loudly-on-missing prevents accidental auth-less deploys. Cannot run in production until replaced. | 2026-05-03 Session 11 (Track Calendar C4) port | **RESOLVED Session 18 (commit `c206bd3`):** `@clerk/nextjs@^7.3.2` installed; new `middleware.ts` runs `clerkMiddleware()` behind a `CLERK_SECRET_KEY`-presence gate so dev/test/preview without Clerk keys still serves via `STUB_USER_ID`; `<ClerkProvider>` conditionally wraps `src/app/layout.tsx` when `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` is set; `getAuthSession()` body resolves Clerk when both keys are set, falls back to `STUB_USER_ID` in dev/test/preview, hard-throws in production with neither configured. 78 route callsites unchanged. 875/875 tests pass. Operator action: add `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` (All Envs) + `CLERK_SECRET_KEY` (Production + Preview only, marked Sensitive) to Vercel. |
...
| W-017 | Q5 `nextRoundType`-driven metamorphism ships without per-investor-class question bias from `Organization` records. The Q5 BUILD_SEQUENCE row referenced "LTM ecosystem data ported in V1," but Track V's V1 spec adds 4 valuation Prisma models (ValuationSubject, ValuationRun, ValuationSensitivity, FinancialSnapshot) — NOT `Organization`. Track V is sequenced *after* Track Q in BUILD_SEQUENCE, and the `project_ltm_types_no_speculative_generalization` rule forbids porting `Organization` data without a real consumer. Q5 currently drives metamorphism off `f23 — Target Round Type` + per-field `investorClassRelevance` only. Investor-bias from real LTM ecosystem records (e.g., "Atomico tends to lead Series A in fintech; Octopus prefers Seed in climate") is the missing layer. | 2026-05-07 Q5 sequencing audit | When Track L (cluesintelligence unification) or Track J (vertical adapters) introduces a real cross-app consumer of LTM ecosystem data, port `Organization` + investor-perspective records via the Brain Enrichment Engine (`docs/03_BRAIN_ENRICHMENT_ENGINE.md`) and replace the static `ROUND_TO_BUYERS` map in `src/lib/quantara/metamorphic/round-buyer-mapping.ts` with a registry-backed `getInvestorClassesForRound` that joins on real-world investor histories. Public function signatures stay identical so Q5 callers don't break. |
