...
| 34 | **Token-streaming chat (Track O O3 — W-003 closed)** | `/api/olivia/chat/stream` route streams cascade tokens as they arrive (AI SDK `streamText` via new `runModelCascadeStream`); HomeComposer reads ReadableStream and fires `onReplyChunk` per chunk so HomeHero blockquote grows live. Mock mode emits the deterministic reply in one chunk so the client UI is identical with/without keys. Falls back to the synchronous `/api/olivia/chat` (full per-provider cascade fallback chain) if the stream errors. Rate limited 30/min/IP. |
| 36 | **Spoke router (orchestration)** | Cheap regex detector classifies every chat message into one of 7 product spokes (FL real estate / international relocation / London tech / xscore comparison / heart recovery / London transit / general fallback). Each non-general spoke carries a substantive system-prompt addendum naming the canonical references investors / clinicians / users at that spoke care about (MLS+RESO+FEMA, Atomico+Index+Level39, MHRA+FDA+NICE, Numbeo+Mercer+183-day, etc.). Detection precedence tuned (heart_recovery > FL > LDN-transit > LDN-tech > relocation > xscore). Streaming chat surfaces the detected spoke as `X-Olivia-Spoke` header; HomeHero renders it as an aether-tinted chip in the provenance row. 16 unit tests. |
| 38 | **Conversation persistence + multi-turn (streaming)** | Streaming chat route now recalls the last 4 turns from the conversation store before each cascade call AND persists user + assistant turns through the SafeConversationStore. conversationIdRef on the client threads the same id across sends; X-Olivia-Conversation-Id response header lets the client pin. "New conversation" command in ⌘K wipes the ref via a CustomEvent. Multi-turn just works without any UI changes. |
| 39 | **Live cascade trace dashboard (`/admin/traces`)** | Bloomberg-style table of the last 30 cascade calls — intent, runtime mode (live/mock), selected provider+model, attempt count, total duration, tool dispatches, response preview. Polls `/api/traces` every 5s. Both production chat routes (`/api/olivia/chat` and `/api/olivia/chat/stream`) now record FoundationTraces (was: only LangGraph experimental route), so the dashboard populates from real traffic. ⌘K nav target. |
| 37 | **Timeline manifest** | `timeline` fence renders chronological narratives — funding history, regulatory milestones, hiring timelines, project plans — as a vertical rail with token-tinted dots (4 tones: neutral/positive/warning/danger), monospace dates, bold titles, and optional detail copy. Distinct from `chart` (measurements over time) — `timeline` is for events. 9 unit tests. |
| 35 | **Production hardening (Track K)** | S27 security audit (no committed secrets, no XSS via dangerouslySetInnerHTML on user input, no SQL injection in raw queries — all parameterized) + rate limits on the 3 cost-vector endpoints (chat-stream / voice-synthesize / voice-transcribe at 20-30/min/IP). S28 Cache-Control + stale-while-revalidate headers on the 3 home aggregator routes (60-80% TTFB drop expected on multi-tab fanout). S29 `docs/RUNBOOK.md` — pre-deploy checklist, env-var inventory, smoke tests, rollback, on-call playbook with 7 common 5xx causes + fixes, cost-vector dashboards. |
...
| **Track N (Visual Manifestation)** — N1+N3+N5+timeline of 5 | 🟡 **PARTIAL** | N1 (manifest contract) ✅. N3 (charts) ✅. N5 (Gamma deck preview) ✅. Plus timeline manifest. **Remaining:** N2 (Mapbox 3D enhancement), N4 (generative UI / 3D scenes). |
...
| **Track O (Weakness closure)** — O3+O4 of 4 | 🟡 **PARTIAL** | **O3 ✅** (W-003 perceived chat/voice latency closed via token streaming). **O4 ✅** (W-004 citation-first RAG closed at manifest layer). O2 (Patronus eval runtime), O5 (avatar lip-sync upgrade) remaining. |
| **Track K (Hardening + launch prep)** — S27+S28+S29 | ✅ **CLOSED** | S27 ✅ security audit + rate limits on cost vectors. S28 ✅ perf caching headers (60-80% TTFB drop expected). S29 ✅ production `RUNBOOK.md`. |
