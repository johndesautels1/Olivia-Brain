...
> **Last updated:** 2026-05-08 — end of Track P (P3 → P7) batch + Track F (Clerk auth) + Track B Sessions 8 + 8b + 8b-routes (documents rendering + write-surface data layer). **Track P CLOSED (7/7 ✅). Track F CLOSED (1/1 ✅). Track B Sessions 8 + 8b + 8b-routes ▲** — W-009 rendering layer fully closed AND write-surface data layer + 6 API routes shipped; the 14 component ports defer to S8b-routes-components.
> **HEAD will be:** the Track B Session 8b-routes docs commit that ships alongside this file (after `241cc89` post-S8b-routes feat).
> **Test gate:** 927/927 across 85 suites. Typecheck clean.
>
> **Operator action owed:** apply `prisma/sql/08-add-documents-engine-write-surface.sql` to Supabase via SQL Editor (paste-into-Supabase shape, body pasted inline in chat at session close per the inline-SQL rule).
...
**TRACK P + TRACK F + TRACK B (S8 + S8b + S8b-routes data layer) ARE CLOSED.** No specific next session is pre-locked. Olivia Brain's offer-evaluation surface, Clerk auth, the **complete documents rendering layer** (18 blocks + renderer + body + question engine + workspace shell), AND the **documents write-surface data layer** (4 Prisma models + 6 API routes + SQL migration) are all in place. The remaining documents work is the **14 component ports** that consume the routes (BookmarkButton, DocumentActionBar, SaveToPackageModal, ShareDocumentModal, etc.) and the eventual Document Prisma model + app routes.

**Likely candidates per BUILD_SEQUENCE — founder picks priority:**

| Track | Sessions | Why pick this |
|---|---|---|
| **Track B Session 8b-routes-components** (RECOMMENDED next) | 1 | Verbatim ports of the 14 LTM components that consume the routes shipped in S8b-routes. Completes the documents UX end-to-end. Pure ports + Clerk-aware ADAPT files (BookmarkButton, DocumentActionBar use `useUser()`; the conditional ClerkProvider from Track F gates them gracefully). After this, the workspace shell renders bookmark/package/share UI fully wired to real endpoints. |
| **Track B Session 8c** | 1 | **Studio v1 engine** port — PreparationStudio + 17 engine-side components (StudioAnswerEditor, StudioFormattingToolbar, PitchPolishModal, SuggestionChips, WhyThisPanel, DeepResearchPanel, ResearchHistory, EntityBriefCard, EntityPerspectiveModal, MicroReward, SkipNudgeModal, CompletionCeremony, DocumentTransition, PreSubmitCheck, CristianoReEvaluation, AnswerRibbon, StoryReview). Originally the Session 8 deliverable; split out so atoms could ship clean. Independent of 8b-routes-components. |
| **Track B Session 8d** | 1 | Documents **app routes** port (`app/documents/*`, 13 files) + the **Document Prisma model** that the rendering layer's `documentId` strings finally point at + the real fork logic for `/api/me/documents/save-from-template` (currently a 501 stub pointing here). Lands after S8b-routes-components so the workspace shell is mountable from a real route. |
...
**Recommended:** **Track B Session 8b-routes-components** as the natural follow-up — it closes the documents UX end-to-end and makes the routes shipped in this session actually usable from the workspace shell. After that, Track C remainder → Track K hardening is the path to the June 8 demo. The founder may pull Track L forward if cluesintelligence is the demo target, or take Track G first for the cascade orchestrator.
...
- **Documents write-surface data layer is shipped** (Track B Session 8b-routes). 4 Prisma models + 6 API routes are live. **The components that consume them aren't ported yet** (Session 8b-routes-components). Don't try to mount BookmarkButton or DocumentActionBar — they're still in LTM only. The routes themselves are exercisable directly via fetch / curl / Postman against any running OB instance, but require the SQL migration to be applied.
...
- **`userId` column type diverges from the older OB convention.** Earlier OB Prisma models (`CounterTermSheet`, `DealAnalysis`, etc.) use `@db.Uuid` on `userId`. The 4 new documents-write-surface models use plain `String` — Clerk's real user IDs (`user_2x...`) are not UUIDs, and the existing convention would have rejected them once Track F's keys are configured on Vercel. Don't follow the older pattern when adding new user-scoped tables; mirror the new shape.
