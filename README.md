...
| W-009 ▲ rendering closed + write-surface data layer | LTM documents subsystem entanglement — port surfaced 3 missed LTM utility files (`@/types/blocks`, `@/lib/autolinker`, `@/lib/documents/content`); Clerk dependencies in BookmarkButton + DocumentActionBar; OrgMapProvider used by 4 blocks (not 2 as manifest claimed); react-markdown + remark-gfm dependencies; DocumentRenderer routes break when any block defers; cannot ship clean without Clerk strategy | 2026-05-03 Session 7 attempted port | **RENDERING-LAYER CLOSED** across **Session 8** (commit `8d30887`) + **Session 8b** (commit `2abec1a`). **WRITE-SURFACE DATA LAYER + ROUTES SHIPPED** in **Session 8b-routes** (commit `241cc89`): 4 Prisma models (`DocumentBookmark`, `DocumentPackage`, `PackageItem`, `DocumentShare`) + `prisma/sql/08-add-documents-engine-write-surface.sql` + 6 API routes (`/api/documents/bookmark` toggle, `/api/packages` list+create, `/api/packages/documents` add, `/api/documents/[id]/share` list+create, `/api/documents/[id]/share/[shareId]` revoke, `/api/me/documents/save-from-template` 501-stub pending Session 8d Document model). 22 new route smoke tests. **927/927 passing across 85 suites.** Inline regex-divergence comment in OB's `computeBlockStatus` + `computeFieldStatus` documents an OB-only bug fix from S8 (LTM source untouched). **Operator action owed:** apply migration 08 to Supabase. **What still defers (Session 8b-routes-components):** the 14 component ports — DocumentActionBar, BookmarkButton, SaveToPackageModal + AddToPackageButton + PackageProgressBar, ShareDocumentModal, SaveToMyDocumentsButton + useSaveToMyDocuments, DocumentSourcePanel, plus pure-port siblings DocumentFilters / DocumentCard / DocumentQuickView / DocumentQuickViewProvider / PrintButton. App routes (`app/documents/*`, 13 files per manifest § K.4 step 6) are Session 8d. |
