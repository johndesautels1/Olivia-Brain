...
> **Last updated:** 2026-05-08 — end of Track P + Track F + Track B (Sessions 8 + 8b + 8b-routes + 8b-routes-components + 8d data foundation + 8d-routes partial). **Track P CLOSED (7/7 ✅). Track F CLOSED (1/1 ✅). Track B (rendering + write-surface + data foundation + 5/14 app routes) ✅ — W-009 CLOSED.** Documents UX rendering chain complete; Document + UserProfile Prisma models + real fork logic shipped; OB-adapted query modules + 5 simpler app routes shipped. **What's still missing:** 4 heavy app routes (index, detail, saved, workspace) carry forward to S8d-routes-2.
> **HEAD will be:** the Session 8d-routes docs commit that ships alongside this file (after `0d59eb3` post-routes feat).
> **Test gate:** 929/929 across 85 suites. Typecheck clean.
>
> **Operator action owed (5 SQL files in apply order):** 06 deal-protection → seed investor-reputations → 07 counter-term-sheets → 08 documents-engine-write-surface → 09 documents-foundation. **DB unreachable from the Claude dev workstation today** (`db.lumfvloapckluhzvtgdn.supabase.co:5432` returned P1001 — likely free-tier auto-pause; wake the project or paste each via SQL Editor in order, OR run `npx prisma migrate deploy` from a connected terminal).
...
**TRACK P + TRACK F + TRACK B (S8 + S8b + S8b-routes + S8b-routes-components + S8d data foundation + S8d-routes partial) ARE CLOSED — W-009 CLOSED + documents data + access layer + 5/14 app routes shipped.** Olivia Brain's offer-evaluation surface, Clerk auth, the complete documents UX rendering chain, the documents data foundation (Document + UserProfile + enums + FK constraints + real fork logic), 2 OB-adapted query modules, AND 5 simpler documents app routes (loading, error, new, edit, share) are all in place. The remaining documents work is **9 app routes**: 4 heavy ones (index, detail, saved, workspace) defer to S8d-routes-2; 3 studio routes defer to S8c (depend on PreparationStudio).

**Likely candidates per BUILD_SEQUENCE — founder picks priority:**

| Track | Sessions | Why pick this |
|---|---|---|
| **Track B Session 8d-routes-2** (RECOMMENDED next to finish the heavy app routes) | 1 | **4 heavy app routes** (`documents/page.tsx` index 19.5 KB, `documents/[id]/page.tsx` detail 16.9 KB, `documents/saved/page.tsx`, `documents/[id]/workspace/*`). Each requires adapting LTM-only dependencies — index needs PackageCard component port + document-registry constants; detail needs ReadAloudButton + collection grouping fallbacks; workspace needs COLLECTION_DNA_MAP from lib/analysis/constants (not in OB). |
| **Track B Session 8c** | 1 | **Studio v1 engine** port — PreparationStudio + 17 engine-side components (StudioAnswerEditor, StudioFormattingToolbar, PitchPolishModal, SuggestionChips, WhyThisPanel, DeepResearchPanel, ResearchHistory, EntityBriefCard, EntityPerspectiveModal, MicroReward, SkipNudgeModal, CompletionCeremony, DocumentTransition, PreSubmitCheck, CristianoReEvaluation, AnswerRibbon, StoryReview). Originally the Session 8 deliverable; split out so atoms could ship clean. Independent of 8d-routes. |
...
- **Documents UX rendering chain + data foundation are fully ported** (Track B S8 + S8b + S8b-routes + S8b-routes-components + S8d data foundation). Atoms (18 blocks + renderer + body + question engine), workspace shell (DocumentWorkspace + 4 sibling components), write-surface data layer (4 LTM-aligned Prisma models + 6 API routes + SQL migration 08), write-surface components (14 LTM components), AND data foundation (Document + UserProfile Prisma models, 12 LTM-aligned enums, FK constraints, real fork logic at `/api/me/documents/save-from-template`, `ensureUserProfile()` helper at `src/lib/users/`). `<DocumentWorkspace />` mounts end-to-end against any WorkspaceDocument; bookmark/package/share UI is wired to real OB routes; the fork endpoint creates real Document rows. **What's still missing:** the 13 `app/documents/*` route pages that mount the workspace from a real URL (Session 8d-routes). Until S8d-routes ships, the rendering chain works against stub data but not from a real `/documents/[id]` URL.
- **`ensureUserProfile()` is now the canonical lookup-or-create helper** for user-scoped tables that FK to UserProfile. Idempotent via prisma.userProfile.upsert. When you add a new user-scoped route that needs a `userProfileId`, call `ensureUserProfile(authSession.userId)` first — bookmark route is the reference. Routes that store the raw Clerk userId directly (Package.ownerUserId, DocumentShare.ownerUserId) don't need it.
