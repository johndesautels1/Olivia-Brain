 ...
 *
 * **LTM-first audit (2026-05-07):**
 * - Composio: ZERO LTM hits. OB-original is correct.
 * - Approval-gate / confidence-gate: ZERO LTM hits. OB-original is correct.
 * - This dispatch wrapper is therefore the agent-facing layer over the
 *   vendor SDK (`services/composio.ts`), not over LTM tech. The wrapper
 *   architecture is justified because the SDK adapter and the agent
 *   API have distinct responsibilities.
 *
 * **Known future ports:** Track G S19-S20 ports LTM's `lib/cascade/`
 * orchestrator (8 providers, full pipeline). When that lands, the
 * `tools` array hook moves from OB's interim `services/model-cascade.ts`
 * to the ported orchestrator. Track H S21-S23 ports LTM's 94 named
 * agents at `lib/agents/impl/` — those are pre-defined task runners,
 * complementary to (not replacing) this dispatch surface.
 *
