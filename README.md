...
| W-009 ▲ rendering closed | LTM documents subsystem entanglement — port surfaced 3 missed LTM utility files (`@/types/blocks`, `@/lib/autolinker`, `@/lib/documents/content`); Clerk dependencies in BookmarkButton + DocumentActionBar; OrgMapProvider used by 4 blocks (not 2 as manifest claimed); react-markdown + remark-gfm dependencies; DocumentRenderer routes break when any block defers; cannot ship clean without Clerk strategy | 2026-05-03 Session 7 attempted port | **RENDERING-LAYER CLOSED** across **Session 8** (commit `8d30887`) + **Session 8b** (commit `2abec1a`). Session 8 shipped atoms: 18 blocks + DocumentRenderer + DocumentBody + 3 missed utility files + OrgMapProvider + DocumentWorkspace data-spine + lib/studio/engine namespaced; 24 tests. Session 8b shipped workspace shell: DocumentWorkspace component portion + DocumentFieldEditor + DocumentTemplatePreview + WorkspaceOliviaPanel + DocumentEditor; 6 tests. **905/905 passing across 79 suites.** Inline regex-divergence comment in OB's `computeBlockStatus` + `computeFieldStatus` documents an OB-only bug fix (LTM source untouched). **What still defers (Session 8b-routes):** the PORT+ADAPT files that need OB-side API routes + Prisma models — DocumentActionBar (bookmark + share buttons), BookmarkButton (Clerk + `/api/bookmarks`), SaveToPackageModal + AddToPackageButton + PackageProgressBar (`/api/packages` + `DocumentPackage` + `PackageItem`), ShareDocumentModal (`/api/share` + `DocumentShare`), SaveToMyDocumentsButton + useSaveToMyDocuments (`/api/saved-documents`), DocumentSourcePanel (re-points to bridge providers); plus pure-port siblings DocumentFilters / DocumentCard / DocumentQuickView / DocumentQuickViewProvider / PrintButton. App routes (`app/documents/*`, 13 files per manifest § K.4 step 6) are Session 8d. |
