...
| **C2** | **Calendar engine + queries.** `lib/queries/calendar.ts` (35 KB, 93 LTM-domain references — adapt: userProfileId → userId rename + drop linkedEvent/linkedOrg/linkedPerson selects + adjust `CalendarEntryWithDetails` interface). `lib/calendar/*` (19 files: olivia-engine, olivia-prompts, olivia-schemas, calendar-judge, calendar-memory, behavior-engine, document-aware, daily-brief, event-categories, founder-journey, google-sync, outlook-sync, crypto, proximity-cluster, rrule-expand, travel-buffer, workflow-generator, olivia-guardrails, index). Plus the calendar slice of `lib/olivia/tools.ts`. | Adapted queries + 19 engine files typecheck against C1's Prisma client. |
