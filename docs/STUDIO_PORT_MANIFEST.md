...
5. ✅ **Done across Sessions 8 + 8b + 8b-routes (data-layer)** —
   - **Session 8** (commit `8d30887`) shipped 18 blocks + DocumentRenderer + DocumentBody + DocumentWorkspace.tsx **data-spine portion** (types + helpers, LTM lines 1–208).
   - **Session 8b** (commit `2abec1a`) appended the **DocumentWorkspace React component** (LTM lines 257–612) plus the 4 sibling components it imports verbatim: DocumentFieldEditor, DocumentTemplatePreview, WorkspaceOliviaPanel, DocumentEditor. The full split-pane workspace shell now mounts end-to-end.
   - **Session 8b-routes** (commit `241cc89`) shipped the **write-surface data layer + API routes** — 4 Prisma models (`DocumentBookmark`, `DocumentPackage`, `PackageItem`, `DocumentShare`) + `prisma/sql/08-add-documents-engine-write-surface.sql` + 6 API routes (bookmark toggle, packages list/create, packages/documents add, share list/create, share revoke, save-from-template 501-stub). 22 surface-contract tests.

   The **14 component ports that consume those routes** — DocumentActionBar, BookmarkButton, SaveToPackageModal, AddToPackageButton, PackageProgressBar, ShareDocumentModal, SaveToMyDocumentsButton, useSaveToMyDocuments, DocumentSourcePanel, DocumentFilters, DocumentCard, DocumentQuickView, DocumentQuickViewProvider, PrintButton — defer to **Session 8b-routes-components**. The Document Prisma model + the real fork logic for `/api/me/documents/save-from-template` defer to **Session 8d** (documents app routes session); the 501 stub points at it explicitly.
