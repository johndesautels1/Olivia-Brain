...
const generateResponse: typeof Phase1State.Node = async (state) => {
  ...
  // sessions can extend the eligibility logic. INTERIM: this wiring
  // points at OB's `services/model-cascade.ts`; when Track G S19-S20
  // ports LTM's `lib/cascade/` orchestrator, this hook moves there.
  const toolBundle =
